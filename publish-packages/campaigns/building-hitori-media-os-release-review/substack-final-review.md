...
```text
TODO: 公開後に貼り付ける（Sanity Studioで substackPostPlan.publishedUrl にも反映する）
```
...
```text
TODO: 公開後に記入する。subscriber 流入数、返信数、引用、Notesでの会話、X / note からの流入の有無など。
substackGrowthAction.resultNotes にも Sanity Studio から手動反映する。
```
